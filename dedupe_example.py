...
if __name__ == '__main__':
    ...
    ic(len(messy))
    ic(type(messy))
    ic(list(messy.keys())[:10])
    ic(list(messy.values())[:10])
    ...
    if ic(os.path.exists(settings_file)):
    ...
    # Making Index from canonical data
    ...
    ic(len(messy))
    ic(len(canonical))
    ic(len(cluster_membership))
    ic(type(cluster_membership))
    ic(list(cluster_membership.items())[0])
