...
class MatchLearnerBase(ERBase):
    ...
    def start(self):
        ...
        if self._output_storage.check_exists(self.train_file_name):
            self._training_file = self._output_storage._backend.download_core(
                self.train_file_name + '.json')
    ...
    def transform(self, inputs: List[pd.DataFrame],
        ...
        while True:
            ...
            fields = [
                field for field in self._deduper.fingerprinter.index_fields]
...
class CanonMatchLearner(Messy2Canon, MatchLearnerBase):
    ...
    def prepare_training(self, inputs: List[pd.DataFrame]):
        ...
        self._deduper.prepare_training(
            messy, canonical, training_file=self._training_file)
...
class MessyMatchLearner(MessyOnly, MatchLearnerBase):
    ...
    def prepare_training(self, inputs: List[pd.DataFrame]):
        ...
        print('# of Messy Data:', len(messy))
        self._deduper.prepare_training(
            messy, training_file=self._training_file)
