...
class FormatNode(ObjProcessor):
    def __init__(self, node_name: str, input_storage: PandasStorage, output_fs: FileSystem):
    ...
    def transform(self, inputs: List[pd.DataFrame], **kwargs) -> List[pd.DataFrame]:
        ...
        rename_dict = dict([(key, f'{key}:{TYPE_CONVERT[str(value)]}') for key, value in df.dtypes.to_dict().items() if key != 'node_id'])               
        rename_dict.update(
...
class FormatLink(ObjProcessor):
    def __init__(self, link_name: str, from_node: str, to_node: str, input_storage: PandasStorage, output_fs: FileSystem):
    ...
    def transform(self, inputs: List[pd.DataFrame], **kwargs) -> List[pd.DataFrame]:
        ...
        rename_dict = dict([(key, f'{key}:{TYPE_CONVERT[str(value)]}') for key, value in df.dtypes.to_dict().items() if key != 'from_id' and key != 'to_id'])
        rename_dict.update(
            {
